...
plt.xticks(fontsize = fnt)
plt.yticks(fontsize = fnt)
plt.text(3.5*10**-5, 6 , '(b)' )
plt.grid(linewidth = .5)
plt.xlabel('PSR ($s^{-1}$)', fontsize = fnt)
ax2.axvspan(6/fact, 10/fact, ymin=0, ymax=10, alpha=0.3, color=color_clipped, zorder = -10)
plt.ylabel(r'$\tau_{rec}$ (hour)', fontsize = fnt)
plt.ylim( 0, 5.99)
plt.text(.0007, 4.9, 'Clipped\n  data',bbox=dict(facecolor='w', edgecolor='k', boxstyle='round', alpha = .5))
plt.plot([0.00175, .00325] , [ 5.2, 5.5], 'k')
...
plt.ylim(0 ,9.99)
plt.xlim(.1,10)
ax3.axvspan(6, 10, ymin=0, ymax=10, alpha=0.3, color=color_clipped, zorder = -10)
...
plt.xlim(.1,10)
plt.grid(linewidth = .5)
ax4.axvspan(6, 10, ymin=0, ymax=10, alpha=0.3, color=color_clipped, zorder = -10)
plt.xlabel('PGA (m/s$^2$)', fontsize = fnt)
plt.ylabel(r'$\tau_{rec}$ (hour)', fontsize = fnt)
plt.ylim( -.1, 5.99)
ax4.set_xscale('log')
